...
const Trade: React.FC<any> = ({
    ...
    return (
        <div className="w-full h-full flex flex-col justify-start items-center py-5">
            <div className={`${showBatchList ? "!w-full" : "!w-[50%]"} h-full flex flex-col lg:flex-row justify-start lg:justify-center items-center lg:items-start gap-4`}>
                <div className="w-full md:max-w-xl h-full">
                    {showFromSelectionMenu || showToSelectionMenu ? (
                        <div className="w-full max-h-full bg-backgound-300 border border-backgound-600 shadow shadow-backgound-500 flex flex-col gap-2 rounded-lg cursor-pointer p-3">
                            <MdOutlineArrowBack onClick={showFromSelectionMenu ? () => closeFromSelectionMenu() : () => closeToSelectionMenu()} className="rounded-full h-10 w-10 p-2 hover:bg-font-1100 active:bg-font-1000  text-font-100" />
                            <div className="flex flex-col justify-center items-center gap-5 px-4">
                                <div className="flex flex-wrap justify-center items-center gap-2">
                                    {NETWORK_LIST?.map((item) => {
                                        return (
                                            <div
                                                key={item.chainName}
                                                onClick={showFromSelectionMenu ? () => handleSelectFromNetwork(item) : () => handleSelectToNetwork(item)}
                                ...
                                <div className="w-full flex justify-start items-center gap-2 bg-font-100 border-2 border-font-300 rounded-md py-2 px-5">
                                    <input
                                        ...
                                        onChange={showFromSelectionMenu ? (e) => setFilterFromToken(e.target.value) : (e) => setFilterToToken(e.target.value)}
                                ...
                                <div className="w-full overflow-auto flex flex-col justify-center items-center py-1">
                                    {showFromSelectionMenu && selectedFromNetwork.chainName && (
                                        <div className="w-full max-h-96">
                                            {protocolNames[selectedFromNetwork.chainId]?.key.map((item: tTradeProtocol, protocolIndex: number) => {
                                                return protocolNames[selectedFromNetwork.chainId].value[protocolIndex].toLowerCase().includes(filterFromToken.toLowerCase()) ? (
                                                    <div key={item.name} className="w-full py-0.5">
                                                        ...
                                                        >
                                                            <div className="w-full flex justify-start items-center gap-3">
                                                                <Image src={item.icon} alt="" className="h-8 w-8 bg-font-200 rounded-full cursor-pointer" />
                                                                <div>{protocolNames[selectedFromNetwork.chainId].value[protocolIndex]}</div>
                                                            </div>
                                                        ...
                                                        {selectedFromProtocol === item.name && selectedFromProtocol !== "erc20" && (
                                                            <div className="bg-backgound-100 text-font-200 rounded-lg p-3 my-1.5">
                                                                ...
                                                                {isLoadingTokenList ? (
                                                                    <div className="w-full h-20 flex justify-center items-center gap-0">
                                                                        <Image src={loader} alt="loader" className="h-10 w-10 rounded-full animate-spin" />
                                                                    </div>
                                                                ) : tokensSupported?.[selectedFromNetwork.chainName]?.[item.name] ? (
                                                        ...
                                                        {item.name === "erc20" && selectedFromProtocol === "erc20" && fromTokensData && (
                                                            <div className="bg-backgound-100 text-font-200 rounded-lg p-3 my-1.5">
                                                                <div className="w-full flex justify-start items-center gap-2 bg-font-100 rounded-md shadow py-1.5 px-5">
                                                                    <input
                                                                        type="text"
                                                                ...
                                                                {fromTokensData?.map((token: iTokenData, tokenIndex: number) => {
                                                                    return token.symbol.toLowerCase().includes(filterFromAddress.toLowerCase()) ? (
                                                                        <div
                                                                            key={tokenIndex}
                                                                            onClick={() => onChangeFromToken(token.address, token.symbol, "erc20")}
                                    ...
                                    {showToSelectionMenu && selectedToNetwork.chainName && (
                                        <div className="w-full max-h-96">
                                            {protocolNames[selectedToNetwork.chainId]?.key.map((item: tTradeProtocol, protocolIndex: number) => {
                                                return protocolNames[selectedToNetwork.chainId].value[protocolIndex].toLowerCase().includes(filterToToken.toLowerCase()) ? (
                                                    <div key={item.name} className="w-full py-0.5">
                                                        <div
                                                            key={item.name}
                                                        ...
                                                        >
                                                            <div className="w-full flex justify-start items-center gap-3">
                                                                <Image src={item.icon} alt="" className="h-8 w-8 bg-font-200 rounded-full cursor-pointer" />
                                                                <div>{protocolNames[selectedToNetwork.chainId].value[protocolIndex]}</div>
                                                            </div>
                                                            <div className="w-auto flex justify-end items-center gap-2">
                                                        ...
                                                        {selectedToProtocol === item.name && selectedToProtocol !== "erc20" && (
                                                            <div className="bg-backgound-100 text-font-200 rounded-lg p-3 my-1.5">
                                                                {tokensSupported?.[selectedToNetwork.chainName]?.[item.name] && (
                                                                    <div className="flex justify-center items-center gap-3 pb-3">
                                                                ...
                                                                {isLoadingTokenList ? (
                                                                    ...
                                                                    <div className="w-full flex flex-col justify-center items-center gap-0">
                                                                        {Object.values(toSelectedActionTokenList).length > 0 &&
                                                                            Object.values(toSelectedActionTokenList).map((item: any) => (
                                                                                <div
                                                                                    key={item.shareTokenAddress}
                                                                                    onClick={() => onChangeToToken(item.nativeTokenDetails.nativeToken, item.nativeTokenDetails.symbol, item?.type != undefined ? item.type : "")}
                                                                                    className="w-full flex justify-between items-center gap-3 hover:bg-backgound-200 active:bg-backgound-100 py-2 px-3 rounded-lg cursor-pointer my-2"
                                                                                >
                                                                                    <span>
                                                        ...
                                                        {item.name === "erc20" && selectedToProtocol === "erc20" && toTokensData && (
                                                            <div className="bg-backgound-100 text-font-200 rounded-lg p-3 my-1.5">
                                                                <div className="w-full flex justify-start items-center gap-2 bg-font-100 rounded-md shadow py-1.5 px-5">
                                                                    <input
                                                                        type="text"
                                                                ...
                                                                {toTokensData?.map((token: iTokenData, tokenIndex: number) => {
                                                                    return token.symbol.toLowerCase().includes(filterToAddress.toLowerCase()) ? (
                                                                        <div
                                                                            key={tokenIndex}
                                                                            onClick={() => onChangeToToken(token.address, token.symbol, "erc20")}
                        ...
                        <div className="w-full bg-backgound-300 flex flex-col gap-1 shadow shadow-backgound-800 rounded-2xl cursor-pointer">
                            <h1 className="w-full bg-backgound-300 text-font-100 text-lg md:text-xl lg:text-2xl text-center font-bold rounded-t-2xl p-5">Building Batch No. {individualBatch.length}</h1>
                            <div className="w-full flex flex-col gap-5 px-5 py-7">
                                <div
                                    className={`w-full relative flex justify-center items-center gap-5 ${
                                        selectedToNetwork.chainName && selectedToProtocol && selectedToToken && selectedFromNetwork.chainName && selectedFromProtocol && selectedFromToken ? "flex-row" : "flex-col"
                                    }`}
                                >
                                    {/* ---------- FROM Section START ---------- */}
                                    <SelectionBar
                                        ...
                                        subIcon={protocolNames[selectedFromNetwork.chainId]?.key.find((entry: any) => entry.name == selectedFromProtocol)?.icon}
                                    ...
                                    {selectedFromActionType != "Lending" && selectedFromActionType != "Repay" && (
                                        <div onClick={handleSwap} className="absolute flex justify-center items-center border-2 bg-font-200 hover:bg-font-100 active:bg-font-400 rounded-full">
                                            <Image
                                                src={swap}
                                                alt=""
                                                className={`h-10 w-10 p-2 ${
                                                    selectedToNetwork.chainName && selectedToProtocol && selectedToToken && selectedFromNetwork.chainName && selectedFromProtocol && selectedFromToken
                                    ...
                                    {selectedFromActionType != "Lending" && selectedFromActionType != "Repay" && (
                                        <SelectionBar
                                            ...
                                            subIcon={protocolNames[selectedToNetwork.chainId]?.key.find((entry: any) => entry.name == selectedToProtocol)?.icon}
                                ...
                                <div className="bg-backgound-100 border border-backgound-300 rounded-lg px-5 py-3">
                                    <h5 className="text-sm md:text-base lg:text-lg font-medium md:font-semibold text-font-100">You Pay</h5>
                                    <div className="relative flex flex-row justify-start items-center gap-8 py-3">
                                        {selectedFromNetwork.chainName && selectedFromProtocol ? (
                                            <div className="relative">
                                                <Image src={selectedFromNetwork.icon} alt="" className="h-12 w-12 bg-backgound-300 rounded-full cursor-pointer" />
                                                <div className="absolute -bottom-1 -right-1 bg-backgound-100 h-6 w-6 flex justify-center items-center rounded-full">
                                                    <Image
                                                        src={protocolNames[selectedFromNetwork.chainId].key.find((entry: any) => entry.name == selectedFromProtocol)?.icon || defaultBlue}
                                                        alt=""
                                        ...
                                        <div className="text-font-100">
                                            ...
                                            <div className="absolute right-0 bottom-0 flex flex-col justify-center items-end gap-1">
                                                {maxBalance !== amountIn && maxBalance !== "0" ? (
                                                    <span onClick={() => onChangeAmountIn(maxBalance ? maxBalance.toString() : "0")} className="text-xs md:text-sm text-font-100 font-medium bg-button-100 rounded-xl px-3 py-1">
                                                        Max
                                                    </span>
                                                ) : maxBalance == "0" ? (
                                                    <span className="text-xs md:text-sm text-font-100 font-medium bg-red-700 rounded-xl px-3 py-1">No Balance</span>
                                                ) : null}
                                ...
                                <div className="w-full flex justify-center items-center gap-3">
                                    <Button handleClick={() => sendSingleBatchToList(true)} isLoading={addToBatchLoading} customStyle="" innerText="Add Batch to List" />
                                </div>

                                <div className="w-full flex justify-center items-center gap-3">
                                    <Button handleClick={() => handleExecuteMethod()} isLoading={sendTxLoading} customStyle="" innerText="Execute Batch" />
                                    {/* <Button
                ...
                {selectedFromNetwork.chainId && showBatchList && (
                    <div className="w-full md:max-w-2xl max-h-full bg-backgound-300 border border-backgound-600 shadow shadow-backgound-500 flex flex-col justify-start items-center gap-1 rounded-2xl cursor-pointer">
                        <h1 className="w-full bg-backgound-300 text-font-100 text-lg md:text-xl lg:text-2xl text-center font-bold rounded-t-2xl p-5">Batching List</h1>
                        {selectedFromNetwork.chainId && totalfees.gt(0) && (
                            <div className="w-auto flex justify-between items-center gap-10 sm:gap-40 border-2 border-gray-600 rounded-lg mb-3 px-4 py-2.5">
                                <h3 className="flex justify-start items-center gap-1 text-font-100 font-bold text-xs md:text-sm">
                                    <Image src={gas} alt="gas" className="h-6 w-6 mr-1 sm:mr-2" />
                                    <span>Total Gas</span>
                                ...
                                <div className="flex justify-between items-center gap-1">
                                    <h6 className="text-font-100 font-semibold text-sm">
                                        {Number(totalfees).toPrecision(4).toString()} :<span className="px-1 text-font-300 font-medium text-xs">{ChainIdDetails[selectedFromNetwork.chainId].gasFeesName}</span>
                                    </h6>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="w-full max-h-full overflow-auto flex flex-col gap-5 px-5 py-7">
                            {selectedFromNetwork.chainId && individualBatch.length > 0 && individualBatch[0].txArray.length > 0 ? (
                                            ...
                                            <div key={bar.id} className="relative">
                                                <div className="simulation-success flex flex-col justify-center items-start gap-1 bg-backgound-100 p-5 rounded-xl text-black font-medium transition duration-300">
                                                    <div className="w-full flex justify-between items-center gap-2">
                                                        ...
                                                        <MdDelete color="red" size="40px" onClick={() => removeBatch(inputBarIndex)} className="hover:bg-backgound-600 active:bg-backgound-400 p-2 rounded-full" />
                                                    </div>
                                                    <div className="w-full flex flex-col justify-between items-start gap-2 p-3 rounded-xl bg-backgound-300 shadow-sm mt-4" onClick={() => toggleShowBatchList(bar.id)}>
                                                        <div className="w-full flex justify-between items-center gap-2">
                                                            <div className="flex justify-start items-start gap-5">
                                                                <div className="relative">
                                                                    <Image src={ChainIdDetails[selectedFromNetwork.chainId].networkLogo} alt="" className="h-10 w-10 bg-font-200 rounded-full cursor-pointer" />
                                                                    <div className="absolute -bottom-1 -right-1 bg-font-100 h-5 w-5 flex justify-center items-center rounded-full">
                                                                        <Image
                                                                            src={protocolNames[selectedFromNetwork.chainId].key.find((entry: any) => entry.name == bar.data.fromProtocol)?.icon || defaultBlue}
                                                                            alt=""
                                                            ...
                                                            <div className="flex justify-center items-center bg-backgound-500 hover:bg-backgound-100 rounded-full p-0.5">
                                                                {showIndividualBatchList === bar.id ? <MdKeyboardArrowUp size="30px" className="text-font-500" /> : <MdKeyboardArrowDown size="30px" className="text-font-500" />}
                                                            </div>
                                                        </div>
                                                            ...
                                                            <div className="flex flex-col justify-start items-start gap-1 pl-10 pt-3">
                                                                {bar.batchesFlow.length > 0 &&
                                                                    bar.batchesFlow.map((item: any, index: number) => (
                                                                        <div key={item.action} className="flex flex-col justify-start items-start gap-1">
                                                                            <div key={item.action} className="flex justify-center items-center gap-3">
                                                                                <div className="relative">
                                                                                    <Image src={ChainIdDetails[item.fromChainId].networkLogo} alt="" className="h-8 w-8 bg-slate-200 rounded-full cursor-pointer" />
                                                                                    <div className="absolute -bottom-1 -right-1 bg-font-100 h-4 w-4 flex justify-center items-center rounded-full">
                                                                                        <Image
                                                                                            src={protocolNames[item.fromChainId].key.find((entry: any) => entry.name === item.protocol)?.icon || defaultBlue}
                                                                                            alt=""
                                                    ...
                                                    <div className="w-full flex flex-col justify-center gap-1 rounded-lg px-2 mt-3">
                                                        <div className="w-full flex justify-between items-center gap-1">
                                                            <h3 className="w-full text-green-400 font-bold text-xs">Gas Used</h3>
                                                            <h6 className="w-full flex justify-end items-center gap-2 text-font-100 font-semibold text-sm">
                                                                ...
                                                                <span>{Number(bar.data.fees).toPrecision(3).toString()} :</span>
                                                                <span className="text-font-300 font-medium text-xs">{ChainIdDetails[selectedFromNetwork.chainId].gasFeesName}</span>
                                                            </h6>
                                                        </div>
                                                        {Number(bar.data.extraValue) ? (
                                                            <div className="w-full flex justify-between items-center gap-1">
                                                                <h3 className="w-full text-green-400 font-bold text-xs">Extra Native Gas</h3>
                                                                <h6 className="w-full flex justify-end items-center gap-2 text-font-100 font-semibold text-sm">
                                                                    <Image src={gas} alt="gas" className="h-4 w-4 mr-1" />
                                                                    <span>{Number(bar.data.extraValue).toPrecision(3).toString()} :</span>
                                                                    <span className="text-font-300 font-medium text-xs">{ChainIdDetails[selectedFromNetwork.chainId].gasFeesName}</span>
                                                                </h6>
                                                            </div>
                                                        ) : null}
                                ...
                                <div className="text-center text-font-700 font-semibold text-base md:text-lg">{txhash ? "Last Batches executed, Now create new batches" : "No Batches Found !"}</div>
                            )}
                        </div>
                    </div>
                )}
