...
import { useSafeProvider } from "./aaProvider/useSafeProvider";
...
export function useSendTx() {
    ...
    const { mutateAsync: sendToSafe } = useSafeProvider();
    ...
    async function sendTxToChain({ funcIndex, address, isSCW }) {
        try {
            ...
            if (isSCW) {
                ...
                _currentAddress = "0xDDED1AB97Ee6a18D9e69aCE4b5Ca7b7221373d02";
            ...
            if (!BigNumber.from(allowance).gte(BigNumber.from(amountIn))) {
                alert(amountIn.toString());
                alert(fromStarGateRouter);
                const approveData = await USDT.populateTransaction.approve(fromStarGateRouter, amountIn);
                console.log("approveData", approveData);
                approveTx = { to: approveData.to, data: approveData.data, value: BigNumber.from("0").toString() };
            }
            console.log("approveTx", approveTx);

            const transferData = await USDT.populateTransaction.transfer(
                "0xb50685c25485CA8C520F5286Bbbf1d3F216D6989",
                amountIn
            );
            console.log("transferData", transferData);
            const transferTx = { to: transferData.to, data: transferData.data, value: BigNumber.from("0").toString() };
