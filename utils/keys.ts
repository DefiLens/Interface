export const ETHERSCAN_API_KEY = process.env.NEXT_PUBLIC_ETHERSCAN_API_KEY
export const POLYGON_ETHERSCAN_API_KEY =
    process.env.NEXT_PUBLIC_POLYGON_ETHERSCAN_API_KEY
export const infura_key1: any = process.env.NEXT_PUBLIC_POLYGON_INFURA_API_KEY
export const infura_key2 = process.env.NEXT_PUBLIC_POLYGON_INFURA_API_KEY2
export const Biconomy_AA_Key = process.env.NEXT_PUBLIC_BICONOMY_AA_KEY
export const Biconomy_MUMBAI_AA_Key = process.env.NEXT_PUBLIC_BICONOMY_AA_KEY
export const AVAX_ETHERSCAN_API_KEY =
    process.env.NEXT_PUBLIC_AVAX_ETHERSCAN_API_KEY
export const TENDERLY_ACCESS_KEY = process.env.NEXT_PUBLIC_TENDELY_ACCESS_TOKEN
export const TENDERLY_USER = process.env.NEXT_PUBLIC_TENDERLY_USER
export const TENDERLY_PROJECT = process.env.NEXT_PUBLIC_TENDERLY_PROJECT
export const MAINNET_INFURA = process.env.NEXT_PUBLIC_MAINNET_INFURA_API_KEY
