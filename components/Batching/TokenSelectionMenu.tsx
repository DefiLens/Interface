...
const TokenSelectionMenu: React.FC<TokenSelectionMenuProps> = ({
    ...
    return (
        ...
        >
            <div className="relative w-full max-h-[80%] max-w-xl mx-auto rounded-lg z-50">
                <div
                    className={cn(
                        "w-full max-h-full bg-white flex flex-col gap-2 rounded-3xl p-5",
                        showMenu ? "block" : "hidden"
                    )}
                >
                    <button
                        className="absolute top-4 right-4 text-B200 group opacity-50 hover:opacity-100 rounded-full p-1.5 bg-N40 transition-opacity duration-200"
                        onClick={() => closeMenu()}
                    >
                    ...
                    <div className="flex flex-col justify-center items-start gap-3">
                        ...
                        <div className="flex flex-col w-10/12 align-center items-start">
                            ...
                            <h3 className="text-font-600 text-sm lg:text-base font-normal">{description}</h3>
                        </div>
                        {/* Networks Row -- Polygon, Arbitrum, Optimisum, Base */}
                        <div className="w-full flex flex-wrap justify-center items-center gap-5 mb-4">
                            {NETWORK_LIST?.map((item) => {
                                return (
                                    <div
                                        ...
                                        className={cn(
                                            "h-16 w-16 flex justify-center items-center gap-3 bg-font-100 hover:bg-font-200 active:bg-font-400 border-2 border-font-200 hover:border-font-300 shadow-sm rounded-md cursor-pointer",
                                            selectedNetwork?.chainName === item.chainName &&
                                                "bg-gradient-to-br from-[#7339FD] to-[#4DD4F4]"
                                        )}
                                    >
                                        <Image src={item.icon} alt="Network Icons" className="h-12 w-12 rounded-full" />
                                    </div>
                                );
                            })}
                        </div>
