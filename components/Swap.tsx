...
export default function Swap() {
  ...
  const sendTx = async (isScw: boolean) => {
    try {
      ...
      const approveData = await approve({tokenIn, spender: V3_SWAP_ROUTER_ADDRESS, amountIn, address: _address, web3JsonProvider: _provider})
      ...
      if (swapData?.amountOutprice) {
        ...
        if (amountOutByDecimals.eq(0)) {
            ...
            if (BigNumber.from(slippage).gt(0)) {
              const amountAfterSlippage = bg(swapData?.amountOutprice).minus(bg(swapData?.amountOutprice).multipliedBy(slippage).div(100))
            ...
            if (BigNumber.from(slippage).gt(0)) {
              const amountAfterSlippage = bg(amountOutByDecimals).minus(bg(amountOutByDecimals).multipliedBy(slippage).div(100))
              ...
              value={
                amountOutAfterSlippage != 0
                  ? bg(amountOutAfterSlippage)
                      .dividedBy(bg(10).pow(tokenOutDecimals))
                      .toString()
