...
export default function Swap() {
    ...
    const [amountOutAfterSlippage, setAmountOutAfterSlippage] =
        React.useState<any>()
    ...
    const handleSlippage = async (_slippage: any) => {
        ...
        const amountAfterSlippage = bg(amountOut).minus(
            bg(amountOut).multipliedBy(_slippage).div(100)
        )
    ...
    const sendTx = async (isScw: boolean) => {
        try {
            ...
            if (swapData?.amountOutprice) {
                ...
                amountOutByDecimals = amountOutByDecimals.multipliedBy(
                    bg(10).pow(tokenOutDecimals)
                )
                if (amountOutByDecimals.eq(0)) {
                    setAmountOut(swapData?.amountOutprice)
                    if (BigNumber.from(slippage).gt(0)) {
                        const amountAfterSlippage = bg(
                            swapData?.amountOutprice
                        ).minus(
                            bg(swapData?.amountOutprice)
                                .multipliedBy(slippage)
                                .div(100)
                        )
                        console.log(
                            "amountAfterSlippage-1",
                    ...
                    if (BigNumber.from(slippage).gt(0)) {
                        const amountAfterSlippage = bg(
                            ...
                            bg(amountOutByDecimals)
                                .multipliedBy(slippage)
                                .div(100)
                        )
                        console.log(
                            "amountAfterSlippage-2",
                            amountAfterSlippage.toString()
                        )
                        setAmountOutAfterSlippage(amountAfterSlippage)
                    }
    ...
    return (
        <>
                ...
                <div>
                    ...
                    <div style={{margin: "5px"}}>
                        ...
                        <input
                            ...
                            onChange={(e: any) =>
                                handleAmountOut(e.target.value)
                            }
                        />
                    </div>
            ...
            />
                    ...
                    <div style={{margin: "5px"}}>
                        <button onClick={() => sendTx(true)}>
                            SendTx By SCW
                        </button>
                    </div>

                    <div style={{margin: "5px"}}>
                        <button onClick={() => sendTx(false)}>
                            SendTx BT EOA
                        </button>
                    </div>
                </div>
